...
export function generateAndPrintInvoice(order: any) {
  ...
  const itemsHtml = (order.items || []).map((it: any, idx: number) => {
    ...
    const author = it.book?.author || it.author || '';
  ...
  const html = `
    ...
    <html>
      <head>
        <meta charset="utf-8">
        <title>Invoice #${orderNum} - Techno World Books</title>
        <style>
          body {
      ...
      <body>
        ...
        <div style="max-width: 720px; margin: 0 auto; border: 1px solid #cbd5e1; border-radius: 12px; padding: 28px;">
          <!-- Header -->
          <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #e2e8f0; padding-bottom: 20px;">
            ...
            <div style="text-align: right;">
              ...
              <div style="font-size: 13px; font-weight: 700; color: #047857; margin-top: 4px;">#${orderNum}</div>
          ...
          <div style="margin-top: 20px; padding: 14px; border-radius: 8px; background: ${isPickup ? '#f0fdf4' : '#f8fafc'}; border: 1px solid ${isPickup ? '#bbf7d0' : '#e2e8f0'};">
            ...
            ` : `
              <div style="font-size: 12px; font-weight: 800; color: #1e293b;">
                🚚 POSTAL DELIVERY (INDIA POST / COURIER)
              </div>
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 24px;">
            <thead>
              <tr style="background: #f8fafc; border-bottom: 2px solid #cbd5e1;">
                <th style="padding: 10px 8px; text-align: center; font-size: 11px; font-weight: 800; color: #475569; width: 40px;">#</th>
                <th style="padding: 10px 8px; text-align: left; font-size: 11px; font-weight: 800; color: #475569;">ITEM DESCRIPTION</th>
                <th style="padding: 10px 8px; text-align: center; font-size: 11px; font-weight: 800; color: #475569; width: 60px;">QTY</th>
                <th style="padding: 10px 8px; text-align: right; font-size: 11px; font-weight: 800; color: #475569; width: 90px;">RATE</th>
                <th style="padding: 10px 8px; text-align: right; font-size: 11px; font-weight: 800; color: #475569; width: 100px;">TOTAL</th>
              </tr>
            </thead>
            <tbody>
              ${itemsHtml}
